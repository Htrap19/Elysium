...
public:
	...
	inline void TranslateTo(const glm::vec3& pos, float t)
	{
        ...
        m_StartPosition = cc.CameraIns.GetPosition();
		m_EndPosition = pos;
		m_TotalTime = t;
...
protected:
	...
	virtual void OnUpdate(Elysium::Timestep ts) override
	{
		...
		auto& cc = m_Self.GetComponent<Elysium::CameraComponent>();
        cc.CameraIns.SetPosition(m_StartPosition + (m_EndPosition - m_StartPosition) * smoothT);
        cc.CameraIns.SetFront(m_Direction);
	}
